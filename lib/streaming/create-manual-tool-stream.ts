  ...
  return createDataStreamResponse({
    execute: async (dataStream: DataStreamWriter) => {
      ...
      try {
        ...
        // Inject advanced symbol context if enabled
        const baseMessages =
          advancedChatEnabled && advancedChatSymbol
            ? ([
              {
                role: 'system',
                content:
                  `Active trading symbol context: ${advancedChatSymbol}. ` +
                  'Prioritize this market in your analysis unless the user specifies a different asset.'
              },
              ...truncatedMessages
            ] as any)
            : truncatedMessages
        ...
        const researcherConfig = manualResearcher({
          messages: [...baseMessages, ...toolCallMessages],
          model: modelId,
